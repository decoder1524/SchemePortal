                            ...
                            <tr key={item.userId}>
                                ...
                                {allResults?.find(user => user.userId === item.userId) ?
                                    <td className='border border-gray-700 p-2 hover:bg-gray-500 font-bold'>{allResults
                                        ...
                                        ?.eligibleScheme
                                        ?.map(scheme => scheme.schemeName).join('\n').split('\n').join('\n')}</td> : <td className='border border-gray-700 p-2 hover:bg-gray-500 '>Not Eligible</td>}
                                {allResults?.find(user => user.userId === item.userId) ?
                                    <td className='border border-gray-700 p-2 hover:bg-gray-500 font-bold'>{allResults
                                        ?.find(user => user.userId === item.userId)
                                        ?.notEligibleScheme
                                        ?.map(scheme => scheme.schemeName).join('\n').split('\n').join('\n')}</td> : <td className='border border-gray-700 p-2 hover:bg-gray-500 '>Profile Not Found</td>}
                                {allResults?.find(user => user.userId === item.userId) ?
                                    <td className='border border-gray-700 p-2 hover:bg-gray-500 font-bold'>{allResults
                                        ?.find(user => user.userId === item.userId)
                                        ?.notEligibleScheme
                                        ?.map(reasons => reasons.reasons).join(' *')}</td> : <td className='border border-gray-700 p-2 hover:bg-gray-500 '>Profile Not Found</td>}
